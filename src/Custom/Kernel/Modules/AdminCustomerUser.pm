...
# --
# 
# Customization Package WPLinkCompanyToService 
# Custom/Kernel/Modules/AdminCustomerUser.pm 
# Goal: upon CustomerUser save, update also service associations, based on the company
# --
# Oreste Attanasio 2011
# All edits marked with OA
#
#
...
#            $Self->{LogObject}->Log( Priority => 'notice', Message => "\$Update: " . $Update);
	    ...
	    # OA : Edit service association, if CustomerUser's company has some
	    # 1) Delete actual associations
	    # 2) Get all services associated with the company using the Service Object 
	    #	 (CustomerUserServiceMemberList searching another CustomerUser of the same company)
	    # 3) Associate the services to the CustomerUser using the Service Object (CustomerUserServiceMemberAdd)
	     
	    	#OA: The company associated to the newly created customer
		if ($GetParam{UserCustomerID}) {
			# OA: CompanyID is set

			# (1)
			# OA: Delete actual service associations 
			# $GetParam{ID} contains the old userlogin
			# $GetParam{UserCustomerID} contains the new userlogin (IF CHANGED)
			my $result = $Self->{DBObject}->Do( 
				#SQL => "DELETE FROM service_customer_user WHERE customer_user_login IN (SELECT login from customer_user WHERE id = ? ) ", 
				SQL => "DELETE FROM service_customer_user WHERE customer_user_login = ?  ", 
				Bind => [\$GetParam{ID}],
				);
#            		$Self->{LogObject}->Log( Priority => 'notice', Message => Dumper(%GetParam));
            		$Self->{LogObject}->Log( Priority => 'notice', Message => "Delete query:: DELETE FROM service_customer_user WHERE customer_user_login = " . $GetParam{ID}  . " Result: " .   $result);

			# (2)
			# OA: Search services associated customer users in the same company
			my $SQL = "SELECT DISTINCT scu.service_id, s.name " . 
			       "FROM service_customer_user scu, customer_user cu, service s " . 
			       "WHERE scu.customer_user_login = cu.login " .
			       "AND s.id = scu.service_id " . 
			       #"AND cu.customer_id =\"" . $GetParam{UserCustomerID} . "\" ";
			       "AND cu.customer_id = ? ";

			#$Self->{DBObject}->Prepare(SQL => $SQL);
			$Self->{DBObject}->Prepare(SQL => $SQL, Bind => [\$GetParam{UserCustomerID}]);
			
			# (3)
			# OA: associate services to the new customer user
		        while ( my @Row = $Self->{DBObject}->FetchrowArray() ) {
				$Self->{ServiceObject}->CustomerUserServiceMemberAdd(
            		            CustomerUserLogin => $GetParam{CustomerUser},
	                            ServiceID         => $Row[0],
                           	    Active            => 1,
            		            UserID            => $Self->{UserID},
	                        );
				$Self->{LogObject}->Log( Priority => 'notice', Message => "Associated serivce ".$Row[1]. " to customer user " . $GetParam{CustomerUser} );
    ...
    # ------------------------------------------------------------ #
	    ...
	    # OA : Save service association, if CustomerUser's company has some
	    # 1) Get all services associated with the company using the Service Object 
	    #	 (CustomerUserServiceMemberList searching another CustomerUser of the same company)
	    # 2) Associate the services to the CustomerUser using the Service Object (CustomerUserServiceMemberAdd)
	     
	    
		$Self->{LogObject}->Log( Priority => 'notice', Message => "COMPANY = " . $GetParam{UserCustomerID});
		...
		if ($GetParam{UserCustomerID}) {
			...
			#$Self->{DBObject}->Prepare(SQL => $SQL);
			$Self->{DBObject}->Prepare(SQL => $SQL, Bind => [\$GetParam{UserCustomerID}]);
			#$Self->{LogObject}->Log( Priority => 'notice', Message => "asd");
			# OA: associate services to the new customer user
		        while ( my @Row = $Self->{DBObject}->FetchrowArray() ) {
				$Self->{ServiceObject}->CustomerUserServiceMemberAdd(
				...
				$Self->{LogObject}->Log( Priority => 'notice', Message => "Associated serivce ".$Row[1]. " to customer user " . $User );
