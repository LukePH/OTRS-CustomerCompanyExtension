# --
# Kernel/Modules/AdminCustomerUserGroup.pm - to add/update/delete customeruserscompany <-> groups
# Copyright (C) 2011-2011 Wuerth Phoenix
# Thomas Forrer & Oreste Attanasio 06/2012
# --

package Kernel::Modules::AdminCustomerCompanyGroup;
...
sub new {
    ...
    $Self->{GroupBlacklist} = $Self->{ConfigObject}->Get('WPCustomerCompanyExtension::GroupBlacklist') || ['users','admin', 'stats'];
...
sub Run {
    ...
    my %VisibleType = ( CustomerCompany => 'CustomerCompany', Group => 'Group', );
...
#        	$GroupMemberList{$Row[0]} = $Row[1];
#	}

        my %GroupMemberList = $Self->{CustomerGroupObject}->GroupMemberList(
	        UserID => $UsersInCompany[0],
	        Type   => 'rw',
        	Result => 'HASH',
    	);

	# Test: should output all services associated with company
	#$Self->{LogObject}->Log( Priority => 'error', Message => Dumper(\%CompanyGroups) );
#	$Self->{LogObject}->Log( Priority => 'error', Message => Dumper( $UsersInCompany[0] ));
#	$Self->{LogObject}->Log( Priority => 'error', Message => Dumper(\%GroupMemberList) );
#	$Self->{LogObject}->Log( Priority => 'error', Message => Dumper(\%AllUsers) );
#	open $ERRLOG, '>>/tmp/otrs.err.log';
#	print $ERRLOG 'TEST\n';
#	close $ERRLOG;

#        # List services.
#        my %GroupData = $Self->{GroupObject}->GroupList(
#            UserID => $Self->{UserID},
#        );
	...
	# Group blacklisti
	while (my ($gid, $Group) = each(%GroupData))
	{
		$Self->{LogObject}->Log( Priority => 'error', Message => "Group: $Group");	
#		$Self->{LogObject}->Log( Priority => 'error', Message => Dumper($Self->{GroupBlacklist}));
		foreach(@{$Self->{GroupBlacklist}}) {	
#			$Self->{LogObject}->Log( Priority => 'error', Message => "Blacklist item $_");
			if ($_ eq $Group) {

				$Self->{LogObject}->Log( Priority => 'error', Message => "Delete $_");
        ...
        $Output .= $Self->_Change(
            ...
            Data               => \%GroupData,
            Selected           => \%GroupMemberList,
            CustomerUserSearch => $Param{CustomerCompanySearch},
            GroupSearch      => $Param{GroupSearch},
    ...
    # ------------------------------------------------------------ #
#    elsif ( $Self->{Subaction} eq 'AllocateGroup' ) {
#
#	#OA: Show companies instead of customer users (as of original module)	
#
#        # get params
#        $Param{GroupID} = $Self->{ParamObject}->GetParam( Param => "GroupID" );
#
...
#
#        # get service
#        my %Group = $Self->{GroupObject}->GroupGet(
...
#        my %CustomerUserMemberList = $Self->{GroupObject}->CustomerUserGroupMemberList(
#            GroupID       => $Param{GroupID},
#            Result          => 'HASH',
#            DefaultGroups => 0,
#        );
...
#
#        $Self->{DBObject}->Prepare(SQL => $SQL, Bind => [\$Param{GroupID}]);
#	while ( my @Row = $Self->{DBObject}->FetchrowArray() ) {
...
#    }
    ...
    # ------------------------------------------------------------ #
        ...
        my @GroupIDsSelected = $Self->{ParamObject}->GetArray( Param => 'ItemsSelected' );
        my @GroupIDsAll      = $Self->{ParamObject}->GetArray( Param => 'ItemsAll' );

        # create hash with selected ids
        my %GroupIDSelected = map { $_ => 1 } @GroupIDsSelected;

#	$Self->{LogObject}->Log( Priority => 'error', Message => Dumper(\%GroupIDSelected));

	# OA: get all customeruserids (CustomerUserLogin) matching the company 
	#     then recursively apply $Self->{GroupObject}->CustomerUserGroupMemberAdd(..) #see below
	...
	# Contains all customer users
        ...
        for my $GroupID (@GroupIDsAll) {
            my $Active = $GroupIDSelected{$GroupID} ? 1 : 0;

            # set customer user service member
	    foreach(@UsersInCompany) {
		    $Self->{LogObject}->Log( Priority => 'debug', Message => 'Associate group (rw) '.$GroupID.' to customer user ' . $_. ' of company ' . $Param{CustomerID} . ' with status ' . $Active );
#        	    $Self->{GroupObject}->CustomerUserGroupMemberAdd(
#        	        CustomerUserLogin => $_,
#        	        GroupID         => $GroupID,
#        	        Active            => $Active,
#        	        UserID            => $Self->{UserID},
...
#    # OA: Saves associations from AllocateGroup
#    # ------------------------------------------------------------ #
#    elsif ( $Self->{Subaction} eq 'AllocateGroupSave' ) {
#
#	# OA: Procedure: 
...
#
#        # get params
#        $Param{GroupID} = $Self->{ParamObject}->GetParam( Param => "ID" );
#
...
#        );
    ...
    my $NeType      = $Type eq 'Group' ? 'CustomerCompany' : 'Group';
    my %VisibleType = ( CustomerCompany => 'CustomerCompany', Group => 'Group', );
    my %Subaction   = ( CustomerCompany => 'Change', Group => 'GroupEdit', );
    my %IDStrg      = ( CustomerCompany => 'CustomerID', Group => 'GroupID', );
    ...
    return $NoticeStr.$Self->{LayoutObject}->Output(
        TemplateFile => 'AdminCustomerCompanyGroup',
        Data         => \%Param,
    );
}
...
sub _Overview {
    ...
    my %GroupDataSort = %GroupData;

    # add suffix for correct sorting
    for my $GroupDataKey ( keys %GroupDataSort ) {
        $GroupDataSort{$GroupDataKey} .= '::';
    }

    for my $ID (
        sort { uc( $GroupDataSort{$a} ) cmp uc( $GroupDataSort{$b} ) }
        keys %GroupDataSort
        )
    {
